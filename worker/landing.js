export function landingPage(generateAvatar) {
  // Pre-generate some example avatars
  const examples = ['alice', 'bob', 'charlie', 'diana', 'elena', 'frank', 'grace', 'hiro', 'ivan', 'julia', 'kai', 'luna'];
  const exampleSvgs = examples.map(name => ({
    name,
    svg64: btoa(generateAvatar(name, { size: 80 })),
    svgGrad: btoa(generateAvatar(name, { size: 80, variant: 'gradient' })),
  }));
...
<title>Avataurus — Deterministic Avatar Generator</title>
<meta name="description" content="Generate unique, beautiful avatar faces from any string. Same input, same face. No dependencies."/>
...
.demo-avatar svg{border-radius:20%;box-shadow:var(--shadow)}
...
.gallery-item img{border-radius:20%;box-shadow:var(--shadow)}
...
.feature{background:var(--card);border:1px solid var(--border);border-radius:var(--radius);padding:24px;box-shadow:var(--shadow)}
...
<div class="container">
  <header>
    <div class="logo"><span>🦕</span> Avataurus</div>
    <button class="theme-toggle" onclick="toggleTheme()" aria-label="Toggle theme">🌙</button>
  </header>

  <section class="hero">
    <h1>Unique avatars from <span class="highlight">any string</span></h1>
    <p>Deterministic, beautiful, dinosaur-inspired avatar faces. Same input = same face. Zero dependencies. Runs on the edge.</p>
  </section>
  ...
  <section class="section">
    <h2>Gallery</h2>
    <p class="sub">Every string produces a unique face</p>
    <div class="gallery">
      ${exampleSvgs.map(e => `<div class="gallery-item"><img src="data:image/svg+xml;base64,${e.svgGrad}" width="80" height="80" alt="${e.name}"/><span>${e.name}</span></div>`).join('')}
    </div>
  </section>
  ...
  <section class="section">
    ...
    <div class="code-tabs">
      ...
      <button class="code-tab" onclick="showTab(1)">HTML Element</button>
  ...
  <section class="section">
    ...
    <div class="features">
      <div class="feature">
        <div class="icon">⚡</div>
        <h3>Blazing Fast</h3>
        <p>Pure SVG generation with zero external requests. Runs on Cloudflare's edge network in 200+ cities.</p>
      </div>
      <div class="feature">
        <div class="icon">🎯</div>
        <h3>Deterministic</h3>
        <p>Same input always produces the same avatar. No randomness, no databases, no state.</p>
      </div>
      <div class="feature">
        <div class="icon">🦕</div>
        <h3>Unique Style</h3>
        <p>Friendly, dinosaur-inspired faces with spikes, horns, and personality. Not another boring identicon.</p>
      </div>
      <div class="feature">
        <div class="icon">📦</div>
        <h3>Zero Dependencies</h3>
        <p>Vanilla JavaScript. No React, no frameworks. Works everywhere — browser, Node.js, Workers.</p>
      </div>
      <div class="feature">
        <div class="icon">🔌</div>
        <h3>Web Component</h3>
        <p>Drop-in &lt;avatar-us&gt; custom element with Shadow DOM isolation. Works in any framework.</p>
      </div>
      <div class="feature">
        <div class="icon">♾️</div>
        <h3>Cache Forever</h3>
        <p>Immutable by design. Cache responses forever — the same URL always returns the same image.</p>
      </div>
    </div>
  </section>

  <footer>
    <p>Built with 🦕 by <a href="https://github.com/mladen">mladen</a> · <a href="https://github.com/mladen/avataurus">GitHub</a> · MIT License</p>
  </footer>
</div>
...
<!-- With options -->
<img src="https://avataurus.workers.dev/john?size=128" />\`,
  \`<!-- Load the web component -->
<script type="module" src="https://unpkg.com/avataurus"><\\/script>
...
function updateDemo(){
  ...
  // We'll fetch from the worker itself for live demo
  ...
  ['gradient','solid'].forEach(v=>{
    ...
    d.className='demo-variant';
    const i2=document.createElement('img');
    i2.src='/'+encodeURIComponent(name)+'?size=64&variant='+v;
    i2.width=64;i2.height=64;
    i2.style.borderRadius='20%';
    d.appendChild(i2);
  ...
  d2.className='demo-variant';
  const i3=document.createElement('img');
  i3.src='/'+encodeURIComponent(name)+'?size=64&initial=true';
  i3.width=64;i3.height=64;
  i3.style.borderRadius='20%';
  d2.appendChild(i3);
  const sp2=document.createElement('span');
  sp2.textContent='with initial';
  d2.appendChild(sp2);

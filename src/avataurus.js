...
function spikes(idx, size, color) {
  ...
  const variants = [
    ...
    () => {
      const s = size * 0.08;
      return `<polygon points="${cx - s * 2},${top + s * 0.5} ${cx - s},${top - s * 2} ${cx},${top + s * 0.5}"/><polygon points="${cx - s * 0.5},${top - s * 0.2} ${cx},${top - s * 3} ${cx + s * 0.5},${top - s * 0.2}"/><polygon points="${cx},${top + s * 0.5} ${cx + s},${top - s * 2} ${cx + s * 2},${top + s * 0.5}"/>`;
    },
    // Two horns
    () => {
      const s = size * 0.07;
      return `<polygon points="${cx - r * 0.4 - s},${top + s * 2} ${cx - r * 0.4},${top - s * 2.5} ${cx - r * 0.4 + s},${top + s * 2}"/><polygon points="${cx + r * 0.4 - s},${top + s * 2} ${cx + r * 0.4},${top - s * 2.5} ${cx + r * 0.4 + s},${top + s * 2}"/>`;
    },
    // Single horn
    () => {
      const s = size * 0.06;
    ...
    () => {
      ...
      const l = cx - r,
        ri = cx + r;
      return `<circle cx="${l}" cy="${cx - s * 2}" r="${s * 1.3}"/><circle cx="${l - s * 0.5}" cy="${cx + s}" r="${s}"/><circle cx="${ri}" cy="${cx - s * 2}" r="${s * 1.3}"/><circle cx="${ri + s * 0.5}" cy="${cx + s}" r="${s}"/>`;
    },
    // Crown spikes (5)
    () => {
      const s = size * 0.055;
      ...
      for (let i = -2; i <= 2; i++) {
        const x = cx + i * s * 1.5,
          h = i === 0 ? s * 3 : s * 2;
        p += `<polygon points="${x - s * 0.5},${top + s} ${x},${top - h} ${x + s * 0.5},${top + s}"/>`;
      }
      return p;
    },
    // No spikes
...
function ears(idx, size, color) {
  ...
  const variants = [
    ...
    () => {
      ...
      for (let i = 0; i < 3; i++) {
        const y = earY - s + i * s;
        out += `<polygon points="${leftX},${y} ${leftX - s * 0.9},${y + s * 0.4} ${leftX},${y + s * 0.8}" fill="${color}" opacity="0.6"/>`;
        out += `<polygon points="${rightX},${y} ${rightX + s * 0.9},${y + s * 0.4} ${rightX},${y + s * 0.8}" fill="${color}" opacity="0.6"/>`;
      }
      return out;
    },
    // None
...
function faceMarkings(idx, size, color) {
  ...
  const variants = [
    ...
    () => {
      const spots = [
        [cx - r * 0.3, cy - r * 0.2, s * 1.2],
        [cx + r * 0.25, cy - r * 0.1, s * 0.9],
        [cx - r * 0.15, cy + r * 0.2, s * 1.0],
        [cx + r * 0.35, cy + r * 0.15, s * 0.7],
      ];
      return spots
        .map(([x, y, sr]) => `<circle cx="${x}" cy="${y}" r="${sr}" fill="${color}" opacity="0.2"/>`)
        .join('');
    },
    ...
    () => {
      ...
      for (let i = -1; i <= 1; i++) {
        const y = cy + i * r * 0.25;
        out += `<line x1="${cx - r * 0.5}" y1="${y}" x2="${cx + r * 0.5}" y2="${y}" stroke="${color}" stroke-width="${s * 0.8}" stroke-linecap="round" opacity="0.15"/>`;
      }
      return out;
    },
    ...
    () => {
      ...
      return pts
        .map(([x, y]) => `<circle cx="${x}" cy="${y}" r="${s * 0.6}" fill="${color}" opacity="0.25"/>`)
        .join('');
    },
...
function headAccessory(idx, size, _color, accentColor) {
  const cx = size / 2,
    r = size * 0.38,
    top = cx - r;
  const s = size * 0.04;
  const variants = [
    ...
    () => {
      const y = top - s * 0.5;
      const w = s * 3;
      return `<rect x="${cx - w / 2}" y="${y}" width="${w}" height="${s * 1.5}" rx="${s * 0.2}" fill="#FFD700" opacity="0.8"/><polygon points="${cx - w / 2},${y} ${cx - w / 3},${y - s * 1.5} ${cx - w / 6},${y}" fill="#FFD700" opacity="0.8"/><polygon points="${cx - w / 6},${y} ${cx},${y - s * 2} ${cx + w / 6},${y}" fill="#FFD700" opacity="0.8"/><polygon points="${cx + w / 6},${y} ${cx + w / 3},${y - s * 1.5} ${cx + w / 2},${y}" fill="#FFD700" opacity="0.8"/>`;
    },
    // Headband
    () => {
      const y = top + r * 0.15;
      return `<ellipse cx="${cx}" cy="${y}" rx="${r * 0.85}" ry="${s * 1.2}" fill="none" stroke="${accentColor}" stroke-width="${s * 0.8}" opacity="0.6"/>`;
    },
    // Bow
    () => {
      const bx = cx + r * 0.35,
        by = top + s;
      return `<ellipse cx="${bx - s * 1.2}" cy="${by}" rx="${s * 1.2}" ry="${s * 0.8}" fill="${accentColor}" opacity="0.7"/><ellipse cx="${bx + s * 1.2}" cy="${by}" rx="${s * 1.2}" ry="${s * 0.8}" fill="${accentColor}" opacity="0.7"/><circle cx="${bx}" cy="${by}" r="${s * 0.5}" fill="${accentColor}" opacity="0.9"/>`;
    },
    // Leaf
    () => {
      const lx = cx - r * 0.1,
        ly = top - s;
      return `<ellipse cx="${lx}" cy="${ly}" rx="${s * 0.6}" ry="${s * 1.5}" fill="#22C55E" opacity="0.7" transform="rotate(-20 ${lx} ${ly})"/><line x1="${lx}" y1="${ly - s * 1.5}" x2="${lx}" y2="${ly + s * 1.5}" stroke="#15803D" stroke-width="${s * 0.2}" opacity="0.5"/>`;
    },
    // None
    () => '',
    ...
    () => {
      const ry = top + s * 0.5;
      return `<circle cx="${cx}" cy="${ry}" r="${s * 1.8}" fill="none" stroke="${accentColor}" stroke-width="${s * 0.5}" opacity="0.4"/>`;
    },
  ];
  return variants[idx % variants.length]();
}
...
function bellyPatch(idx, size, color) {
  ...
  const variants = [
    ...
    () => {
      const hs = s * 0.5;
      return `<path d="M${cx} ${cy + hs * 1.8} C${cx - hs * 2.5} ${cy - hs * 0.5} ${cx - hs * 1.5} ${cy - hs * 2} ${cx} ${cy - hs * 0.5} C${cx + hs * 1.5} ${cy - hs * 2} ${cx + hs * 2.5} ${cy - hs * 0.5} ${cx} ${cy + hs * 1.8}Z" fill="${color}" opacity="0.15"/>`;
    },
    // None
    () => '',
  ];
    ...
    () => {
      ...
      for (let x = step; x < size; x += step)
        for (let y = step; y < size; y += step)
          d += `<circle cx="${x}" cy="${y}" r="${size * 0.008}" fill="${color}" opacity="0.15"/>`;
      return d;
    },
    () => '',

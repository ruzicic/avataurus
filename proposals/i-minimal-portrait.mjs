#!/usr/bin/env node
// Proposal I: "Minimal Portrait" - subtle face outline + eyes + mouth
...
// Face shapes: circle, oval, slightly squarish
const faceShapes = [
  (cx, cy) => `<circle cx="${cx}" cy="${cy}" r="42" fill="white" fill-opacity="0.12"/>`,
  (cx, cy) => `<ellipse cx="${cx}" cy="${cy}" rx="38" ry="46" fill="white" fill-opacity="0.12"/>`,
  (cx, cy) => `<rect x="${cx-38}" y="${cy-42}" width="76" height="84" rx="20" fill="white" fill-opacity="0.12"/>`,
];
...
function generate(seed) {
  ...
  const ft = (h >> 5) % faceShapes.length;
  const eyeSize = range(h, 3, 10, 16);
  const eyeSpacing = range(h, 5, 13, 22);
  const faceCy = 62;
  const eyeY = faceCy - range(h, 7, 6, 16);
  const mouthY = faceCy + range(h, 11, 12, 22);
  const cx = 64;
